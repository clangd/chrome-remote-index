...
# Create one server image and push it to the GCR first and create an instance
...
createHC "global" $HEALTH_CHECK_NAME
createHC "europe-west1" $HEALTH_CHECK_NAME
...
LB_NAME="${IG_NAME}-lb"
createLB $LB_NAME "europe-west1" 50051 $HEALTH_CHECK_NAME $NAMED_PORT
...
LB_NAME="${IG_NAME}-lb"
createLB $LB_NAME "global" 5900 $HEALTH_CHECK_NAME $NAMED_PORT
